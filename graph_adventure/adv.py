...
loop_count = 0
while looprun:
    looprun = False  # Comment out this line to run continuously to search for a better path.
    loop_count += 1
    player.currentRoom = world.startingRoom
    traversalPath.clear()
    visited = {}
    ...
    if looprun:  # Loops through and saves best runs if it finds one better than the one saved in best_route.txt
        ...
        if len(traversalPath) < best_length:
            ...
            print(f"Time to find path {len(traversalPath)} long: {time.time() - start_time}")
            print(f"Number of attempts to find it: {loop_count}")
            looprun = False
